...
export class StorachaStorageService {
  ...
  /** 
   *
   * 上传到 Pinata IPFS @see https://docs.pinata.cloud/api-reference/endpoint/upload-a-file#body-group-id
   */
  async uploadToPinata(file: Buffer, filename: string) {
    try {
      ...
      const GATEWAY_DOMAIN = this.pinataGatewayDomain;
      ...
      return {
        cid,
        filename,
        gatewayUrl: `https://${GATEWAY_DOMAIN}/ipfs/${cid}`, // 主网关
  ...
  async getFileInfo(file_id: string) {
    try {
      // 验证文件是否存在于 Pinata
      const response = await fetch(`${this.pinataFileUrl}/public/${file_id}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${this.pinataJwtToken}`,
        }
      ...
      this.logger.error(`Failed to get file info for CID: ${file_id}`, error.stack);
      throw new NotFoundException(`File with CID ${file_id} not found`);
    }
  }
  ...
  async deleteFile(file_id: string) {
    ...
    try {
      ...
      this.logger.log(`File unpinned ${file_id} successfully: ${file_id}`);
      return { message: 'File deleted successfully', file_id };
    } catch (error) {
      this.logger.error(`Failed to delete file: ${file_id}`, error.stack);
      throw error;
    }
  }
