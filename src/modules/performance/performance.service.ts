...
export class PerformanceService {
  ...
  async reportPerformance(
    ...
    try {
      ...
      this.logger.log(`性能数据已保存: ID=${saved.id}, URL=${saved.url}`);
  ...
  async queryPerformance(query: QueryPerformanceDto) {
    ...
    this.logger.debug(`查询性能数据: 共${total}条, 返回${data.length}条`);
  ...
  async getPerformanceStats(query: PerformanceStatsDto) {
    ...
    // 按维度分组统计
    const groupedStats = this.groupAndCalculateStats(metrics, groupBy);
  ...
  private calculateStats(metrics: PerformanceMetric[]) {
    ...
    const median = (arr: number[]) => {
      const filtered = arr.filter(v => v != null && !isNaN(v)).sort((a, b) => a - b);
    ...
    return {
      lcp: {
        avg: Number(avg(validMetrics.map(m => Number(m.lcp)).filter(v => !isNaN(v))).toFixed(2)),
        median: Number(median(validMetrics.map(m => Number(m.lcp))).toFixed(2)),
        p75: Number(percentile(validMetrics.map(m => Number(m.lcp)), 75).toFixed(2)),
        p90: Number(percentile(validMetrics.map(m => Number(m.lcp)), 90).toFixed(2)),
        p95: Number(percentile(validMetrics.map(m => Number(m.lcp)), 95).toFixed(2)),
      },
      fcp: {
        avg: Number(avg(validMetrics.map(m => Number(m.fcp)).filter(v => !isNaN(v))).toFixed(2)),
        median: Number(median(validMetrics.map(m => Number(m.fcp))).toFixed(2)),
        p75: Number(percentile(validMetrics.map(m => Number(m.fcp)), 75).toFixed(2)),
        p90: Number(percentile(validMetrics.map(m => Number(m.fcp)), 90).toFixed(2)),
        p95: Number(percentile(validMetrics.map(m => Number(m.fcp)), 95).toFixed(2)),
      },
      fid: {
        avg: Number(avg(validMetrics.map(m => Number(m.fid)).filter(v => !isNaN(v))).toFixed(2)),
        median: Number(median(validMetrics.map(m => Number(m.fid))).toFixed(2)),
        p75: Number(percentile(validMetrics.map(m => Number(m.fid)), 75).toFixed(2)),
        p90: Number(percentile(validMetrics.map(m => Number(m.fid)), 90).toFixed(2)),
        p95: Number(percentile(validMetrics.map(m => Number(m.fid)), 95).toFixed(2)),
      },
      cls: {
        avg: Number(avg(validMetrics.map(m => Number(m.cls)).filter(v => !isNaN(v))).toFixed(4)),
        median: Number(median(validMetrics.map(m => Number(m.cls))).toFixed(4)),
        p75: Number(percentile(validMetrics.map(m => Number(m.cls)), 75).toFixed(4)),
        p90: Number(percentile(validMetrics.map(m => Number(m.cls)), 90).toFixed(4)),
        p95: Number(percentile(validMetrics.map(m => Number(m.cls)), 95).toFixed(4)),
      },
      ttfb: {
        avg: Number(avg(validMetrics.map(m => Number(m.ttfb)).filter(v => !isNaN(v))).toFixed(2)),
        median: Number(median(validMetrics.map(m => Number(m.ttfb))).toFixed(2)),
        p75: Number(percentile(validMetrics.map(m => Number(m.ttfb)), 75).toFixed(2)),
        p90: Number(percentile(validMetrics.map(m => Number(m.ttfb)), 90).toFixed(2)),
        p95: Number(percentile(validMetrics.map(m => Number(m.ttfb)), 95).toFixed(2)),
      },
  ...
  async deleteOldMetrics(beforeDate: Date): Promise<number> {
    ...
    this.logger.log(`删除了 ${result.affected} 条旧的性能数据`);
